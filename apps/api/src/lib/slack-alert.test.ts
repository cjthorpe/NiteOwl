...
describe("formatPrMergeAlert", () => {
  ...
  it("includes a context block with date string", () => {
    ...
    if (context?.type === "context") {
      const hasDate = context.elements.some((e) =>
        e.text.includes("2024") || e.text.includes("NiteOwl"),
      );
...
describe("sendSlackAlert", () => {
  ...
  it("does NOT retry on 400 (permanent error)", async () => {
    ...
    await expect(sendSlackAlert(webhookUrl, message, 3)).rejects.toBeInstanceOf(
      SlackAlertError,
    );
  ...
  it("does NOT retry on 404 (permanent error)", async () => {
    ...
    const err = await sendSlackAlert(webhookUrl, message, 3).catch(
      (e: unknown) => e,
    );
