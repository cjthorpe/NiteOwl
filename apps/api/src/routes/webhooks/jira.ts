...
export const jiraWebhookRoutes: FastifyPluginAsync<{ db: Db }> = async (
  fastify,
  { db },
) => {
  ...
  fastify.post<{
    ...
    async (request, reply) => {
      ...
      if (!secret) {
        request.log.error(
          "[jira-webhook] JIRA_WEBHOOK_SECRET not configured",
        );
      ...
      try {
        ...
        request.log.info(
          { payloadHash },
          "[jira-webhook] Duplicate delivery, acknowledging",
        );
      ...
      if (!selfStr) {
        request.log.warn(
          "[jira-webhook] Missing issue.self — cannot resolve integration",
        );
      ...
      try {
        ...
        request.log.warn(
          { selfStr },
          "[jira-webhook] Malformed issue.self URL",
        );
        await markWebhookStatus(db, payloadHash, "failed");
        return reply.code(200).send({ status: "skipped" });
      }
      ...
      if (!integration) {
        request.log.warn(
          { siteUrl },
          "[jira-webhook] No enabled integration for site",
        );
        await markWebhookStatus(db, payloadHash, "failed");
      ...
      if (!activity) {
        request.log.debug(
          { eventType },
          "[jira-webhook] Event not actionable, skipping",
        );
