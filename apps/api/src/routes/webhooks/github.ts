...
export const githubWebhookRoutes: FastifyPluginAsync<GitHubWebhookOptions> = async (
  fastify,
  opts,
) => {
  const { db, queue } = opts;
  ...
  fastify.post<{
    ...
    async (request, reply) => {
      ...
      const webhookId = await recordWebhookReceived(db, deliveryId, hash, eventType);
      ...
      try {
        ...
        if (!resolved) {
          ...
          request.log.debug({ senderId }, 'No active GitHub integration for sender — skipping');
          await markWebhookProcessed(db, webhookId, 'processed');
          return reply.code(200).send({ ok: true });
        }
        ...
        request.log.info({ userId, eventType, deliveryId }, 'GitHub webhook enqueued');
