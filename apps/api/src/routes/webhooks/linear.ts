...
export const linearWebhookRoutes: FastifyPluginAsync<{ db: Db }> = async (fastify, { db }) => {
  ...
  fastify.post<{ Body: Buffer }>(
    ...
    async (request, reply) => {
      ...
      if (!secret) {
        request.log.error('[linear-webhook] LINEAR_WEBHOOK_SECRET not configured');
        return reply.code(500).send({ error: 'Webhook not configured' });
      }

      const signature = request.headers['linear-signature'];
      if (typeof signature !== 'string' || signature.length === 0) {
        return reply.code(400).send({ error: 'Missing linear-signature header' });
      }
      ...
      try {
        ...
        request.log.info({ payloadHash }, '[linear-webhook] Duplicate delivery, acknowledging');
      ...
      if (!organizationId) {
        request.log.warn('[linear-webhook] Missing organizationId in payload');
      ...
      if (!integration) {
        request.log.warn({ organizationId }, '[linear-webhook] No enabled integration for org');
        await markWebhookStatus(db, payloadHash, 'failed');
