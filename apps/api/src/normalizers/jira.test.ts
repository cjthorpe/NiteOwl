...
describe('normalizeJiraEvent — invalid payloads', () => {
  it('returns null for an unknown webhookEvent', () => {
    expect(
      normalizeJiraEvent(
        { webhookEvent: 'jira:sprint_started', issue: issueCreatedPayload.issue },
        USER_ID,
      ),
    ).toBeNull();
  });
...
describe('normalizeJiraEvent — comment_created', () => {
  it('normalizes a comment_created event', () => {
    ...
    expect(result?.title).toBe('[PROJ] Comment on PROJ-42: Login page crashes on Firefox 124');
    expect(result?.description).toBe('Looks good — merging now.');
    expect(result?.url).toBe('https://acme.atlassian.net/browse/PROJ-42?focusedCommentId=50001');
