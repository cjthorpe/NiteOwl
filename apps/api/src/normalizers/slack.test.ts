...
const threadedMessagePayload = {
  ...
  event: {
    ...
    permalink: 'https://acme.slack.com/archives/C01ENGINEERING/p1710506000654321',
  },
};
...
describe('normalizeSlackEvent — plain message', () => {
  it('normalizes a plain channel message', () => {
    ...
    expect(result?.description).toBe('Hey team, the deploy just went out to prod!');
    expect(result?.url).toBe('https://acme.slack.com/archives/C01GENERAL/p1710505200123456');
...
describe('normalizeSlackEvent — skipped event types', () => {
  ...
  it('returns null for non-event_callback type', () => {
    expect(normalizeSlackEvent({ type: 'url_verification', challenge: 'abc' }, USER_ID)).toBeNull();
  });
